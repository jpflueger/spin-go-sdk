...
import (
	redistypes "github.com/fermyon/spin-go-sdk/generated/fermyon/spin/redis-types"
...
type Interface interface {
	Del(address string, keys cm.List[string]) cm.OKResult[int64, Error]
	Execute(address string, command string, arguments cm.List[RedisParameter]) cm.OKResult[cm.List[RedisResult], Error]
	Get(address string, key string) cm.OKResult[Payload, Error]
	Incr(address string, key string) cm.OKResult[int64, Error]
	Publish(address string, channel string, payload Payload) cm.ErrResult[struct{}, Error]
	Sadd(address string, key string, values cm.List[string]) cm.OKResult[int64, Error]
	Set(address string, key string, value Payload) cm.ErrResult[struct{}, Error]
	Smembers(address string, key string) cm.OKResult[cm.List[string], Error]
	Srem(address string, key string, values cm.List[string]) cm.OKResult[int64, Error]
}
