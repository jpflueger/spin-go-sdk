...
// Package sqlite represents the interface "fermyon:spin/sqlite".
package sqlite
...
// Connection represents the type "fermyon:spin/sqlite#connection".
//
...
// Close represents function "fermyon:spin/sqlite#close".
//
...
func Close(conn Connection) {
	wasmimport_Close(conn)
}

//go:wasmimport fermyon:spin/sqlite close
//go:noescape
func wasmimport_Close(conn Connection)

// Execute represents function "fermyon:spin/sqlite#execute".
//
...
//
//go:nosplit
func Execute(conn Connection, statement string, parameters cm.List[Value]) cm.OKResult[QueryResult, Error] {
	var result cm.OKResult[QueryResult, Error]
	wasmimport_Execute(conn, statement, parameters, &result)
	return result
}

//go:wasmimport fermyon:spin/sqlite execute
//go:noescape
func wasmimport_Execute(conn Connection, statement string, parameters cm.List[Value], result *cm.OKResult[QueryResult, Error])

// Open represents function "fermyon:spin/sqlite#open".
//
...
//
//go:nosplit
func Open(database string) cm.ErrResult[Connection, Error] {
	var result cm.ErrResult[Connection, Error]
	wasmimport_Open(database, &result)
	return result
}

//go:wasmimport fermyon:spin/sqlite open
//go:noescape
func wasmimport_Open(database string, result *cm.ErrResult[Connection, Error])

type Interface interface {
	Close(conn Connection)
	Execute(conn Connection, statement string, parameters cm.List[Value]) cm.OKResult[QueryResult, Error]
	Open(database string) cm.ErrResult[Connection, Error]
}
