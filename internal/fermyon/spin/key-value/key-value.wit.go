...
type Interface interface {
	Close(store Store)
	Delete(store Store, key string) cm.ErrResult[struct{}, Error]
	Exists(store Store, key string) cm.ErrResult[bool, Error]
	Get(store Store, key string) cm.ErrResult[cm.List[uint8], Error]
	GetKeys(store Store) cm.ErrResult[cm.List[string], Error]
	Open(name string) cm.ErrResult[Store, Error]
	Set(store Store, key string, value cm.List[uint8]) cm.ErrResult[struct{}, Error]
}
